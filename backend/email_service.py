...
"""
    ...
    try:
        response = requests.post(
            ...
            files=[
                ("attachment", (pdf_filename, io.BytesIO(pdf_content), "application/pdf"))
            ],
            timeout=30
        )

        if response.status_code == 200:
            print(f"✅ Delivery email with PDF sent to {to_email}")
            return True
        else:
            print(f"❌ Failed to send delivery email. Status: {response.status_code}")
            print(f"Response: {response.text}")
            return False
